...
if(isset($_GET['id'])){
    $qry = $conn->query("SELECT * FROM `source_list` where id = '{$_GET['id']}'");
    if($qry->num_rows > 0){
        $res = $qry->fetch_array();
        foreach($res as $k => $v){
            if(!is_numeric($k))
...
<div class="container-fluid">
    <form action="" id="source-form">
        ...
        <div class="form-group">
            <label for="name" class="control-label">Source Name</label>
            <input type="text" name="name" id="name" class="form-control form-control-border" placeholder="Enter source Name" value ="<?php echo isset($name) ? $name : '' ?>" required>
        </div>
        <div class="form-group">
